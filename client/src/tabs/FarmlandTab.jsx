...
export default function FarmlandTab({ stats }) {
  const [listings, setListings] = useState([]);
  const [loading, setLoading] = useState(true);
  const [featureFilters, setFeatureFilters] = useState({
    'feature:water': false,
    'feature:solar': false,
    'feature:outbuilding': false,
  });
  ...
  useEffect(() => {
    setLoading(true);
    getListings({ type: 'farmland', sort, order })
  ...
  return (
    <>
      <div className="criteria-banner">
        <strong>Farmland criteria:</strong> 2,500+ sqft home · 5+ acres · within 3 hrs of Ivins, UT
        <span className="criteria-sub">Preferred: water rights / creek, solar, workshop or barn</span>
      </div>

      <div className="stats-row">
        <div className="stat-card">
          <div className="label">Farmland matches</div>
          <div className="value">{stats?.farmland || 0}</div>
      ...
      {loading ? (
        <div className="loading"><div className="spinner" />Loading farmland...</div>
      ) : filtered.length === 0 ? (
        <div className="empty-state">
          <h3>No farmland matches</h3>
